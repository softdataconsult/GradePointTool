---
title: "Grade Point Tool"
format: html
editor: visual
---

## Grade Processing Toolkit

### Code (Processing Function)

```{r}
# ============================
# GradePointTool — full script
# ============================
# Install once:
# install.packages(c("readxl","openxlsx","dplyr","janitor","readr","ggplot2","tidyr","stringr"))
...
# Make column names Excel safe and unique even case-insensitively
...
# Detect a component column like "prac_50" or "prac_20" and its max value
find_component <- function(df, prefix) {
  cand <- names(df)[grepl(paste0("^", prefix, "_\\d+$"), names(df))]
  if (length(cand)) {
    mx  <- as.integer(stringr::str_match(cand, "_(\\d+)$")[,2])
    i   <- which.max(mx)
    list(col = cand[i], max = mx[i])
  } else if (prefix %in% names(df)) {
    list(col = prefix, max = NA_integer_)
  } else {
    list(col = NA_character_, max = NA_integer_)
  }
}

# Plots (viridis palettes)
...
# ----------------------------
# Main processor (single course file)
# ----------------------------
process_exam_sheet <- function(in_path,
                               out_path = sub("(\\.[^.]+)$", "_with_grades.xlsx", in_path),
                               sheet = 1,
                               clamp_0_100 = TRUE,
                               save_plots = TRUE,
  ...
  # Read and normalize
  raw <- read_excel(in_path, sheet = sheet)
  df  <- raw %>% clean_names()  # e.g., "PRAC (20)" -> prac_20; "TOTAL (100)" -> total_100
  ...
  # Detect component columns and maxima
  P <- find_component(df, "prac")
  C <- find_component(df, "ca")
  E <- find_component(df, "exam")

  # Build or coerce Total_100
  if (is.na(total_col) && all(!is.na(c(P$col, C$col, E$col)))) {
    p <- to_num(df[[P$col]]); c <- to_num(df[[C$col]]); e <- to_num(df[[E$col]])
    sum_max <- sum(P$max, C$max, E$max, na.rm = TRUE)

    if (!any(is.na(c(P$max, C$max, E$max))) && sum_max != 100) {
      # Scale to 100 when maxima do not sum to 100
      df <- df %>%
        mutate(Total_100 = round((p / P$max + c / C$max + e / E$max) * 100, 0))
    } else {
      # Simple sum covers 50/10/40 and 20/20/60 or missing maxima
      df <- df %>% mutate(Total_100 = p + c + e)
    }
  ...
  out <- df %>%
    mutate(
      ...
      GradePoint = round(grade_point(Total_100), 2),  # two decimals
      Grade      = letter_grade(Total_100)
    ) %>%
    relocate(Name, Matric_No, Total_100, GradePoint, Grade) %>%
    dedupe_source_cols(name_col, matric_col, total_col, P$col, C$col, E$col) %>%
    fix_names_excel()

  # Summaries with Percent
  ...
  if (write_excel_tables) {
    ...
    # GradePoint 2 decimals in Excel
    ...
    message("Saved Excel with tables: ", out_path)
  }
...
# ----------------------------
# Batch: many courses to separate folders
# ----------------------------
sanitize_course <- function(x) {
  x <- gsub("[^A-Za-z0-9._-]+", "_", x)
  sub("^_+|_+$", "", x)
}

process_many_courses <- function(input_paths,
                                 out_root = "results",
                                 sheet = 1,
                                 save_plots = TRUE,
                                 write_excel_tables = TRUE) {
  ...
  for (in_path in input_paths) {
    course_raw <- file_path_sans_ext(basename(in_path))
    course     <- sanitize_course(course_raw)
    out_dir    <- file.path(out_root, course)
    ...
    res <- process_exam_sheet(
      ...
      write_excel_tables = write_excel_tables
    )

    sf <- res$summary_fine
    getc <- function(g) { v <- sf$Count[sf$Grade == g]; if (length(v)) v else 0 }
    row <- tibble::tibble(
      Course   = course,
      Students = nrow(res$data),
      Aplus = getc("A+"), A = getc("A"),
      Bplus = getc("B+"), B = getc("B"),
      Cplus = getc("C+"), C = getc("C"),
      D = getc("D"), E = getc("E"), F = getc("F"),
      OutputDir = out_dir,
      Workbook  = out_path
    )
    all_rows[[course]] <- row
  ...
  readr::write_csv(summary_all, file.path(out_root, "index_summary.csv"))
...
# Single file:
# res <- process_exam_sheet(
#   in_path  = "data/STA389_scores.xlsx",
#   out_path = "results/STA389_scores_with_grades.xlsx",
#   save_plots = TRUE,
#   write_excel_tables = TRUE
# )

# Many courses:
files <- c("data/STA357_scores.xlsx","data/MTH432_scores.xlsx","data/GNS219_scores.xlsx")
summary_all <- process_many_courses(files, out_root = "results")
print(summary_all)
